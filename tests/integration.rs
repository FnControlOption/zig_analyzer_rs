...
impl Test {
    ...
    fn parse_source<T: Into<Vec<u8>>>(
        ...
        let path: Rc<Path> = Rc::from(path.unwrap_or_else(|| {
            self.counter += 1;
            PathBuf::from(format!("/foo/bar{}.zig", self.counter))
        }));
        let tree: Rc<Ast> = Rc::from(tree);
        let document = Document::new(tree.clone());
        self.documents.insert(path.clone(), document);

        let handle = Handle(path.clone(), tree.clone());
        let root = Node(handle, NodeIndex::ROOT);

        Analyzer::new(&mut self.ip, &mut self.cache, &mut self.documents, root)
    }
...
fn test_decl_access_var() {
    ...
    check! {
        ...
        && test.format_type(ty) == "struct { const Foo = @This(); }"
