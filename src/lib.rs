...
impl InternPool {
    ...
    pub fn get_type_mut(&mut self, index: u32) -> Option<&mut InternedType> {
        self.types.get_index_mut2(index as usize)
    }
...
impl AnalyzerCache {
    ...
    pub fn get(&self, node: &Node) -> Option<&Binding> {
        self.map.get(node)
    }

    pub fn insert(&mut self, node: Node, binding: Binding) -> Option<Binding> {
        self.map.insert(node, binding)
    }
...
pub struct Analyzer<'ip, 'cache, 'doc, 'std> {
    pub ip: &'ip mut InternPool,
...
impl<'ip, 'cache, 'doc, 'std> Analyzer<'ip, 'cache, 'doc, 'std> {
    ...
    fn resolve_binding_uncached(&mut self, node: &Node) -> Binding {
        ...
        match tree.node_tag(node.index()) {
            NodeTag::Identifier => self.resolve_identifier(node, None),
            ...
            NodeTag::FieldAccess => self.resolve_member_access(node, None),
    ...
    pub fn resolve_identifier(
        &mut self,
        node: &Node,
        member_out: Option<&mut Option<Member>>,
    ) -> Binding {
        let handle = node.handle();
        let tree = handle.tree();
        let token = tree.node_main_token(node.index());
        let bytes = tree.token_slice(token);
            ...
            && let Some(bits) = parse_str(tail)
        {
            ...
            let expr = Expr::from(Type::Int(signedness, bits));
            return Binding::Constant(expr);
        }
        ...
        assert!(Rc::ptr_eq(document.tree(), tree));
        let mut opt_member = None;
        for doc_node in document.enclosing_nodes(token) {
            let Some(scope) = &doc_node.scope else {
                continue;
            };
            let Some(&member) = scope.members.get(bytes) else {
                continue;
            };
            opt_member = Some(member);
        }
        let Some(member) = opt_member else {
            return Binding::Unknown;
        };
        member_out.map(|m| *m = Some(member));
    ...
    pub fn resolve_decl(&mut self, handle: &Handle, member: Member) -> Binding {
        match member {
            ...
            Member::Variable(variable) => self.resolve_variable_access(handle, variable),
            Member::Function(function) => self.resolve_function_access(handle, function),
            Member::FunctionParameter(param) => self.resolve_function_param_access(handle, param),
            Member::OptionalPayload(condition, _) => {
                self.resolve_optional_payload_access(handle, condition)
            }
            Member::ErrorUnionPayload(condition, _) => {
                self.resolve_error_union_payload_access(handle, condition)
            }
            Member::ErrorUnionError(condition, _) => {
                self.resolve_error_union_error_access(handle, condition)
            }
        }
    }

    fn resolve_variable_access(&mut self, handle: &Handle, variable: NodeIndex) -> Binding {
        let tree = handle.tree();
        ...
        let expr = match (
            ...
            (Some(ty_index), init) => {
                ...
                match (is_const, init) {
                    (true, Some(init_index)) => {
                        let expr = self.resolve_expr(Node(handle.clone(), init_index));
                        Expr(ty, expr.value())
                    }
            ...
            (None, Some(init_index)) => {
                let Expr(ty, val) = self.resolve_expr(Node(handle.clone(), init_index));
                match is_const {
                    true => Expr(ty, val),
        ...
        // HACK: If this is a type declaration and the type's name is unknown,
        //       set its name based on this declaration.
        if is_const
            && let Expr(Type::Type, Value::Type(Type::Interned(index))) = expr
    ...
    fn resolve_optional_payload_access(
        ...
        let node = Node(handle.clone(), condition);
        let binding = self.resolve_binding(node);
        binding.unwrap_optional(&self.ip)
    }

    fn resolve_error_union_payload_access(
        &mut self,
        handle: &Handle,
        condition: NodeIndex,
    ) -> Binding {
    ...
    fn resolve_error_union_error_access(
        &mut self,
        handle: &Handle,
        condition: NodeIndex,
    ) -> Binding {
    ...
    pub fn resolve_field(&mut self, val: Value, handle: Handle, field: NodeIndex) -> Expr {
        let tree = handle.tree();
    ...
    fn resolve_builtin_call(&mut self, node: &Node) -> Expr {
        let handle = node.handle();
        let tree = handle.tree();
        let main_token = tree.node_main_token(node.index());
        ...
        match tree.builtin_call_tag(node.index()).unwrap() {
            ...
            BuiltinFnTag::This => Expr::from(self.resolve_builtin_call_this(handle, main_token)),
    ...
    fn resolve_builtin_call_import(&mut self, handle: &Handle, args: &[NodeIndex]) -> Expr {
        ...
        let (directory, filename) = match string.as_bytes() {
            b"std" => {
                let Some(std_dir) = self.std_dir else {
                    return Expr::from(Type::Unknown);
                };
                (std_dir, "std.zig")
            }
            b"builtin" => return Expr::from(Type::Unknown), // TODO
            ...
            bytes if bytes.ends_with(b".zon") => return Expr(Type::Unknown, Value::Unknown),
            _ => return Expr::from(Type::Unknown), // bytes => todo!("{}", String::from_utf8_lossy(bytes)),
        };
        let path = Rc::<Path>::from(directory.join(filename));
    ...
    fn resolve_builtin_call_this(&mut self, handle: &Handle, main_token: TokenIndex) -> Type {
        let Some(document) = self.documents.get(handle.path()) else {
            return Type::Unknown;
        };
        let Some(container) = document.enclosing_container(main_token) else {
            return Type::Unknown;
        };
        self.resolve_type(Node(handle.clone(), container.index))
    }
    ...
    fn resolve_fn_proto(&mut self, node: &Node) -> Type {
        ...
        let fn_type = FunctionType {
            ...
            return_type: match fn_proto.ast.return_type.to_option() {
                Some(index) => self.resolve_type(Node::from(handle, index)),
                None => Type::Unknown,
            },
        };
    ...
    fn resolve_container_decl(&mut self, node: &Node) -> Type {
        ...
        'tuple: {
            ...
            for &member in members {
                ...
                let field_ty = match container_field.ast.type_expr.to_option() {
                    Some(index) => self.resolve_type(Node::from(handle, index)),
                    None => Type::Unknown,
                };
                types.push(field_ty);
    ...
    pub fn resolve_from_token(
        ...
        match tag {
            ...
            | NodeTag::ContainerField => {
                ...
                let expr = self.resolve_field(Value::Runtime, handle.clone(), node.index());
                Some(expr)
            }
